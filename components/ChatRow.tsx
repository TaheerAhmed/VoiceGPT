...
const deleteChat=async()=>{
    ...
    return (
        <Link href={`/chat/${id}`} className={`chatRow justify-center text-white ${active && 'bg-gray-500'} m-1.5`}>
            <ChatBubbleLeftIcon className='h-5 w-5' />
            <p className='flex-1 md:inline-flex truncate'>
                {messages?.docs[messages?.docs.length-1]?.data().text||"New Chat"}
            </p>
            <TrashIcon className='h-5 w-5 text-white hover:text-red-300 ' onClick={()=>deleteChat()} />
        </Link>
    )
}
